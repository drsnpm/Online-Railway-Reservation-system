<?php
    ...
    if(isset($_POST['update_train']))
    {
            ...
            $query="update orrs_train set name= ?, route = ?, date = ?, departure_time = ?, arrival_time = ?, number = ?, fare = ?, passengers = ? where id = ?";
            $stmt = $mysqli->prepare($query);
            //bind this parameters
            $rc=$stmt->bind_param('ssssssssi', $name, $route, $date, $departure_time, $arrival_time, $number, $fare, $passengers, $id);
            $stmt->execute();
                if($stmt)
                {
                    $succ = "Train Updated";
                ...
                <div class="card-body">
                  <form method ="POST">
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-12 col-sm-8 col-lg-6">
                        <input class="form-control" name="date" value ="<?php echo $row->date;?>"  id="inputText3" type="date" min="<?php echo date('Y-m-d'); ?>" requirwd>
                      </div>
                    </div>
                    <div class="form-group row">
                      <label class="col-12 col-sm-3 col-form-label text-sm-right" for="inputText3">Departure Time</label>
                      <div class="col-12 col-sm-8 col-lg-6">
                        <input class="form-control" name="departure_time" value ="<?php echo $row->departure_time;?>"  id="inputText3" type="time" required>
                      </div>
                    </div>
                    <div class="form-group row">
                      <label class="col-12 col-sm-3 col-form-label text-sm-right" for="inputText3">Arrival Time</label>
                      <div class="col-12 col-sm-8 col-lg-6">
                        <input class="form-control" name="arrival_time" value = "<?php echo $row->arrival_time;?>" id="inputText3" type="time" required>
                      </div>
                    </div>
                    <div class="form-group row">
