...
if (isset($_POST['Update_Password'])) {
    $aid = $_SESSION['admin_id'];
    $admin_pwd_old = sha1(md5($_POST['admin_pwd'])); // Old password provided by user
    $admin_pwd_new = sha1(md5($_POST['admin_new_pwd'])); // New password provided by user
    
    // Check if old password matches
    $query_select_pwd = "SELECT admin_pwd FROM orrs_admin WHERE admin_id = ?";
    ...
    if ($stmt_select_pwd->num_rows > 0) {
        $stmt_select_pwd->bind_result($admin_pwd_db);
        $stmt_select_pwd->fetch();
        
        if ($admin_pwd_old === $admin_pwd_db) { // Check if old password matches the one in the database
            if ($admin_pwd_old !== $admin_pwd_new) { // Check if new password is different from old password
                // Update password
                $query_update_pwd = "UPDATE orrs_admin SET admin_pwd = ? WHERE admin_id = ?";
                $stmt_update_pwd = $mysqli->prepare($query_update_pwd);
                $stmt_update_pwd->bind_param('si', $admin_pwd_new, $aid);
                  ...
                  <form method ="POST" >
                    <div class="form-group row">
                      ...
                      <div class="col-12 col-sm-8 col-lg-6">
                        <input class="form-control" name="admin_pwd" id="inputText3" type="password" placeholder="enter old password" required>
                      </div>
                    </div>
                    <div class="form-group row">
                      <label class="col-12 col-sm-3 col-form-label text-sm-right" for="inputText3">New Password</label>
                      <div class="col-12 col-sm-8 col-lg-6">
                        <input class="form-control" name="admin_new_pwd"  id="inputText3" type="password" placeholder="enter new password" minlength="8" required>
                      </div>
                    </div>
                    <!-- <div class="form-group row">
