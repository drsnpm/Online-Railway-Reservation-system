 ...
 <?php
	...
	if(isset($_POST['Create_Profile'])) {
		...
		$query_check_email = "SELECT * FROM orrs_passenger WHERE pass_email = ? AND pass_id != ?";
		$stmt_check_email = $mysqli->prepare($query_check_email);
		$stmt_check_email->bind_param('si', $pass_email, $pass_id);
		$stmt_check_email->execute();
		$result_check_email = $stmt_check_email->get_result();

		// If email already exists for other users, set an error message
		if($result_check_email->num_rows > 0) {
			$err = "Email address is already registered for another user";
		} else {
			// Email doesn't exist for other users, proceed with updating the profile
			...
			if ($stmt->execute()) {
				$success = "Passenger's Account Updated";
			} else {
