...
if (isset($_POST['Update_Password'])) {
    $aid = $_SESSION['pass_id'];
    $pass_pwd_old = sha1(md5($_POST['pass_pwd'])); // Old password provided by user
    $pass_pwd_new = sha1(md5($_POST['pass_new_pwd'])); // New password provided by user
    
    // Check if old password matches
    $query_select_pwd = "SELECT pass_pwd FROM orrs_passenger WHERE pass_id = ?";
    ...
    if ($stmt_select_pwd->num_rows > 0) {
        $stmt_select_pwd->bind_result($pass_pwd_db);
        $stmt_select_pwd->fetch();
        
        if ($pass_pwd_old === $pass_pwd_db) { // Check if old password matches the one in the database
            if ($pass_pwd_old !== $pass_pwd_new) { // Check if new password is different from old password
                // Update password
                $query_update_pwd = "UPDATE orrs_passenger SET pass_pwd = ? WHERE pass_id = ?";
                $stmt_update_pwd = $mysqli->prepare($query_update_pwd);
                $stmt_update_pwd->bind_param('si', $pass_pwd_new, $aid);
                ...
                <div class="card-body">
                  <form method ="POST" >
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-12 col-sm-8 col-lg-6">
                        <input class="form-control" name="pass_new_pwd"  id="inputText3" type="password" minlength="8" required>
                      </div>
                    </div>
                    <!-- <div class="form-group row">
