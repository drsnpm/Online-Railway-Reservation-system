  ...
  // }
    if(isset($_POST['train_fare_confirm_checkout'])){
      if (isset($_SESSION['train_fare_confirm_checkout_clicked_time']) && (time() - $_SESSION['train_fare_confirm_checkout_clicked_time']) <= 10) {
        ...
        $_SESSION['train_fare_confirm_checkout_clicked_time'] = time();
            ...
            $query="insert into orrs_train_tickets (pass_id, pass_name, pass_addr, pass_email, train_name, train_no, train_dep_stat, train_arr_stat, train_dep_time, train_fare, fare_payment_code) values (?,?,?,?,?,?,?,?,?,?,?)";
            $stmt = $mysqli->prepare($query); //prepare sql and bind it later
            $rc=$stmt->bind_param('sssssssssss',$pass_id, $pass_name, $pass_addr, $pass_email, $train_name, $train_no, $train_dep_stat, $train_arr_stat, $train_dep_time, $train_fare, $fare_payment_code);
            $stmt->execute();
    ...
    if($stmt) {
        ...
        $query_clear_pass_fare_payment = "UPDATE orrs_passenger 
                                  SET pass_fare_payment_code = '', pass_train_number = '', pass_train_name = '', pass_dep_station = '', pass_dep_time = '', pass_arr_station = '', pass_train_fare = ''
                                  WHERE pass_id = ?";
                ...
                <div class="card-body">
                  ...
                  <div class="form-group row">
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-12 col-sm-8 col-lg-6">
                        <input class="form-control" readonly name="train_dep_time" value="<?php echo $row->pass_dep_time;?>" id="inputText3" type="text">
                      </div>
                    </div>             
                    <div class="form-group row">
